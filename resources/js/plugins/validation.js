...
defineRule('unique_email_tenant', async (value, [url, tenantId]) => {
    ...
    try {
        const response = await axios.post(url, { email: value, tenant_id: tenantId });
    return response.data?.unique === true ? true : 'Este email já está em uso neste tenant.';
    } catch (e) {
...
defineRule('uuid_v7', (value) => {
    ...
    const regex = /^[0-9a-f]{8}-7[0-9a-f]{3}-[0-9a-f]{4}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
    if (!regex.test(value)) {
